...
const socket: Socket | null = typeof window !== 'undefined' ? io({
  path: '/api/socket',
  ...
  reconnection: true,
  reconnectionAttempts: 3, // Reduced attempts to minimize build noise
...
// Enhanced debugging for connection events - only on client side
if (socket) {
  socket.on('connect', () => {
  ...
  socket.on('connect_error', (error) => {
    ...
    if ('type' in error && error.type === 'TransportError') {
      ...
      if (error.message === 'xhr poll error') {
        ...
        // Suppress during build/SSR to reduce noise
        if (typeof window !== 'undefined') {
          console.log('Polling transport error (temporary) - Socket.IO will retry automatically');
        }
        return;
      }
      // Suppress transport negotiation logs during build
      if (typeof window !== 'undefined') {
